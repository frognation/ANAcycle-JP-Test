...
// RD-01: temporary distinct look (effect implementation comes later)
const RD01_RENDER = {
  background: '#0033ff',
  strokeAlphaMin: 0.25,
  strokeAlphaMax: 0.95,
...
class ImageMetaballSystem {
  ...
  animate() {
    ...
    // Clear canvas
    this.ctx.fillStyle = RD01_RENDER.background;
    ...
    this.ctx.globalCompositeOperation = RD01_RENDER.composite;
  ...
  renderMarchingSquares(field, colors, mouseGridX, mouseGridY, mouseInfluenceRadius) {
    for (let y = 0; y < this.gridHeight - 1; y++) {
      for (let x = 0; x < this.gridWidth - 1; x++) {
        ...
        // RD-01: white lines with alpha driven by sampled brightness
        const brightness = (cellColor.r + cellColor.g + cellColor.b) / (3 * 255);
        const alpha = RD01_RENDER.strokeAlphaMin + brightness * (RD01_RENDER.strokeAlphaMax - RD01_RENDER.strokeAlphaMin);
